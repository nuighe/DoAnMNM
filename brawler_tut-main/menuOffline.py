...
class MenuOffline():
    ...
    def createMenu(self):
        while self.run:
            ...
            self.screen.blit(text_surface, (self.SCREEN_W - self.SCREEN_W * 0.8, self.SCREEN_H - self.SCREEN_H * 0.2))
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_RETURN:
                        self.run = False
                    elif event.key == pygame.K_a and self.ft > 1:
                        self.ft = self.ft - 1
                    elif event.key == pygame.K_d and self.ft < 3:
                        self.ft = self.ft + 1 
                    elif event.key == pygame.K_LEFT and self.ft2 > 1:
                        self.ft2 = self.ft2 - 1
                    elif event.key == pygame.K_RIGHT and self.ft2 < 3:
                        self.ft2 = self.ft2 + 1     
